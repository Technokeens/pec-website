<main>
    <section class="top-banner position-relative full-screen pt-5 pb-3 pb-xl-5 d-flex align-items-center justify-content-left banner-height">
      <?php if(!empty($application['banner_image'])) { ?>
        <img loading="lazy" src="<?=base_url();?>uploads/application_banner/<?=$application['banner_image']?>" alt="" class="position-absolute w-100 h-100 left-0 top-0 object-fit-cover">
        <?php }else{ ?>
      ...
      <div class="my-3 my-xl-5 py-3 py-xl-5 px-3 px-xl-5 position-relative banner_data">
        <div class="service-promotion__icon mb-4 hidden-xs">
          <?php if(!empty($application['application_icon_white'])) { ?>
            <img src="<?=base_url();?>uploads/application/white/<?=$application['application_icon_white']?>" class="hicon">
          <?php } ?>
        </div>
        <h2 class="fs-80 fw-semi-bold text-capitalize text-white text-left"><?=(!empty($application['title'])) ? ucfirst($application['title']) : ''; ?></h2>
      ...
      <div class="container">
        <div class="d-flex justify-content-between mb-2 pb-md-2">
          <div class="breadcrumb mb-0 d-none d-md-block flex-grow-1">
            <a href="<?=base_url();?>application-list" class="menu-link menu-link_us-s text-capitalize fw-medium" >Application</a>
